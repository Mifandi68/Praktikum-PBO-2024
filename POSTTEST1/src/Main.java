...
public class Main{
    ...
    private static void addMember(Scanner tambah) {
        ...
        switch (pilihanMembership) {
            case 1:
                jenisMembership = Membership.BRONZE;
                break;
            case 2:
                jenisMembership = Membership.SILVER;
                break;
            case 3:
                jenisMembership = Membership.GOLD;
                break;
            default:
                System.out.println("Pilihan tidak valid. Silakan masukkan pilihan yang benar.");
                break;
        }
        tambah.nextLine(); // consume newline
        System.out.print("Masukkan Tanggal Mulai (YYYY-MM-DD): ");
        String tanggalMulai = tambah.nextLine();

        Membership membership = new Membership(memberId, jenisMembership, tanggalMulai);
